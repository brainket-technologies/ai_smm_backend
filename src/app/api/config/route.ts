...
export async function GET(request: Request) {
  ...
  try {
    ...
    // We only use fallback if DB has no active static pages.
    const dynamicStaticPages = staticPagesRecords.length > 0 
      ? staticPagesRecords.map(page => ({
          id: page.slug,
          title: page.title,
          url: `${appConfig?.apiBaseUrl || "https://ai-smm-backend.vercel.app"}/pages/${page.slug}`
        }))
      : defaultStaticPages;

    // 8. Feature Config Construction (Using appConfig.featuresJson or falling back to detailed object)
    ...
    // Attempt to merge appFeatureFlag records if any
    ...
    if (appConfig?.featuresJson && typeof appConfig.featuresJson === 'object' && Object.keys(appConfig.featuresJson).length > 0) {
      ...
      // If featureFlags table is populated, apply those overrides
      Object.keys(features).forEach(module => {
        if (!dynamicFeatures[module as keyof typeof dynamicFeatures]) {
          (dynamicFeatures as any)[module] = {};
        }

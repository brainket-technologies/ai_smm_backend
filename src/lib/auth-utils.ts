  ...
  if (!apiKey || apiKey !== expectedApiKey) {
    return {
      ...
      response: NextResponse.json(
        { res: "error", message: 'Invalid or missing API Key' },
        { status: 401 }
      ),
    };
  }
...
export async function validateAuth(request: Request) {
  ...
  if (!authHeader?.startsWith('Bearer ')) {
    return {
      isValid: false,
      response: NextResponse.json({ res: "error", message: 'Authorization token required' }, { status: 401 })
    };
  }
  ...
  if (!decoded) {
    return {
      isValid: false,
      response: NextResponse.json({ res: "error", message: 'Invalid or expired token' }, { status: 401 })
    };
  }
  ...
  if (!currentDeviceId) {
    return {
      isValid: false,
      response: NextResponse.json({ res: "error", message: 'device-id header is required for this session' }, { status: 401 })
    };
  }
  ...
  if (!device || device.tokenVersion !== decoded.version) {
    return {
      isValid: false,
      response: NextResponse.json({ res: "error", message: 'Session expired or logged in from another device' }, { status: 401 })
    };
  }
...
export function validateBusinessId(request: Request) {
  ...
  if (!businessId) {
    return {
      isValid: false,
      response: NextResponse.json(
        { res: "error", message: 'x-business-id header is required' },
        { status: 400 }
      ),
    };
  }
  ...
  try {
    ...
    return {
      isValid: false,
      response: NextResponse.json(
        { res: "error", message: 'Invalid x-business-id format' },
        { status: 400 }
      ),
    };
  }
